# language: es

Característica: descifrar el mensaje
  Del sitio world of wargame
  De la categoría kryptos
  Con mi usuario b1nary

  Antecedentes:
    Dado estoy registrado en el sitio world of wargame
    Y tengo el sistema Windows 7
    Y tengo acceso a internet
 
  Escenario: Solución exitosa
    Dado unos numeros queue corresponden a letras
    Cuando se reemplazan por las letras correspondientes
    Entonces obtengo la solución
	Y es THE SECRET PACKAGE IS IN THE BROWN BAG NEAR THE RIVER
	Y el reto es solucionado.
